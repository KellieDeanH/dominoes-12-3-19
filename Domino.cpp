...
//se o load tenha sido feito sem usar o boost � preciso ver quem come�a primeiro, porque a round come�a do inicio
		...
		while (!checkIfRoundIsFinish()) {
			...
			if (checkIfRoundIsBlocked()) {
				...
				cout << "\n  >>>  The round was blocked by no other player having valid bones and no more Boneyard bones!\n" << endl;
				utils::getUserInput();
				break;
			}
			...
			if (!firstBone) {
				while (!playerHasValidBone) {
					...
					if (gameBoneYard.empty()) {
						...
						cout << "\n" << getCurrentPlayer().getName() << " There are no bones to play with and there are no more bones available for purchase!\nTurning the turn...\n\n";
						playerPassedTurn();
						utils::getUserInput();
						break;
					}
					...
					cout << "\n" << getCurrentPlayer().getName() << " There are no possible bones to play with!\nBuying bone from boneyard ...\n\n";
...
void Domino::addPlayersCLI() {
	size_t numPlayers = utils::getNumberCin("  >>> How many players do you want the game to have: ", 5, 2, false, "\nThe game must have between 2 and 4 players!\n\n");

	cout << endl;

	for (size_t i = 0; i < numPlayers; ++i) {
		cout << "\n  >>> Enter player name with no spaces:" << /*i <<*/ ": ";
		string name = utils::getlineCin();

		bool playerIsAI = utils::getYesNoCin("\n  >>> Do you want the player to be AI controlled (Y/N): ", "\nEnter Y for the player to be AI controlled ,\nor N if you want a human to control you!\n\n");
...
void Domino::showBoardAndPlayerStatus() {
	...
	cout << "\n" << getCurrentPlayer().getName() << " have " << getCurrentPlayer().getScore() << (getCurrentPlayer().getScore() == 1 ? " Score" : " points") << ".\n";
	cout << "Number of bones no boneyard: " << gameBoneYard.getBonePile().size() << "\n";
	cout << "AI controlled: " << (getCurrentPlayer().getAIPlayer() ? "Yes" : "No") << "\n" << endl;
...
void Domino::currentPlayerTurnCLI(bool firstBone) {
	...
	do {			
		...
		if (!getCurrentPlayer().getAIPlayer()) {
			...
			if (firstBone) {
				...
				if (getCurrentPlayer().playerHasDoubleBones() && !boneSelected->isDoubleBone()) {
					cout << "\nDuring the first move in the game if you a bone with doubles you must play it!\n\n";
			...
			cout << "AI selected the bone |" << AIBestBoneConectionFound.boneToInsert->getFirstNumber() << "|" << AIBestBoneConectionFound.boneToInsert->getSecondNumber() << "|";
			cout << " to insert into " << getBoardInsertionPointDescription(AIBestBoneConectionFound.wingOnBoardToInsertBone) << endl;

			if (firstBone) {
				boneConectionPointSelected = BONE_CONECTION_POINT_LEFT;
			} else {
				cout << "Select where you want to insert this bone..." << endl;
				boneConectionPointSelected = selectBoneConectionPointToUseFromPlayer();
			}
		...
		if (playerUseInvalidBone) {
			cout << "\n\nCould not insert the bone you specified in the position indicated!\n\n";
			utils::getUserInput();
			utils::clearConsoleScreen();
			showBoardAndPlayerStatus();
		}
	...
	cout << "\n\n\nNumber of points made in this play: " << newPoints << endl;
	
	//s� os multiplos de  � que contam para a pontua�o
	if ((newPoints > 0) && ((newPoints % 5) == 0)) {
		getCurrentPlayer().addPoints(newPoints);
		cout << "\nHave been added " << newPoints << " points toward your score!\n";
		cout << getCurrentPlayer().getName() << " You now have " << getCurrentPlayer().getScore() << " points!\n" << endl;
	}
...
size_t Domino::selectBoneToUseFromPlayer() {
	...
	cout << "\n   >>> Enter the bone number you want to use (S to exit and G to save and exit)\n";

	do {
		if (invalidInput)
			cout << "\nEnter one of the indexes of the bones shown above (or S / G)!\n\n";
...
BoardInsertPoint Domino::selectBoardInsertionPointToUseFromPlayer() {
	...
	colNames.push_back("Board Insertion Points");
	...
	insertionPointIndex = utils::getNumberCin("\n  >>> Insertion Point Index: ", 5, 0, false, "\nEnter one of the indexes in the table!\n\n");
...
BoneConectionPoint Domino::selectBoneConectionPointToUseFromPlayer() {
	...
	string title("");
	vector <string> colNames;
	colNames.push_back("Connection points on selected bone");
	vector < vector <string> > columns;
	columns.push_back(vector <string>());
	...
	utils::printMenu(title, columns, colNames);
	insertionPointIndex = utils::getNumberCin("\n  >>> Connection point index (4 to save the game, 5 to exit): ", 6, 0, false, "\nEnter one of the indexes in the table!\n\n");
...
void Domino::calcAndShowScoresOnRoundEnd(bool roundBlocked) {
	...
	if (roundBlocked) {
		for (size_t i = 0; i < gamePlayers.size(); ++i) {
			cout << " * " << gamePlayers[i].getName() << " had in your hand bones" << calcSumPlayerBonesPoints(gamePlayers[i]) << " points at the end of the round!" << endl;
		}
	}

	cout << "\nOpponents total points: " << bonus;
	cout << "\nRounded opponents total points: " << roundedBonus << endl;

	cout << "---------------------------------------------------------------------------------\n\n";

	cout << gamePlayers[winnerPlayerIndex].getName() << " won this round and got " << roundedBonus << " Bonus points from your opponents!\n" << endl;

	cout << "*********************************************************************************\n";
	cout << "Scores: " << endl;
	for (size_t i = 0; i < gamePlayers.size(); ++i) {
		cout << " -> " << gamePlayers[i].getName() << " : " << (gamePlayers[i].getScore()) << ((gamePlayers[i].getScore() != 1) ? " points" : " score") << "\n";
	}
	cout << "*********************************************************************************\n";
...
void Domino::showScoresOnRoundBlock() {
	...
	cout << "  >>>                 SCORES AT THE END OF THIS ROUND BLOCKED                <<<  \n";
	cout << "#################################################################################\n\n\n";
...
void Domino::showScoresOnRoundEnd() {
	...
	cout << "############################   DOMINO - ALL FIVES   #############################\n";
	cout << "  >>>                     SCORES AT THE END OF THIS ROUND                     <<<  \n";
	cout << "#################################################################################\n\n\n";
...
void Domino::showScoresOnGameEnd() {
	...
	cout << gamePlayers[winnerPlayerIndex].getName() << " score gotten this game!\n" << endl;
...
bool Domino::loadDominoCLI() {
	utils::clearConsoleScreen();
	cout << "\n  >>> Enter the name of the file where the game you want to load is stored.: ";
	...
	try {
		...
		if (utils::getYesNoCin("\nDo you want to try again with another name (Y/N)? ", "\nEnter Y or N!\n\n"))
...
bool Domino::saveDominoCLI() {
	utils::clearConsoleScreen();
	cout << "\n  >>> Enter the file name where you want to save the game.: ";
	string filename = utils::getlineCin();
...
#ifdef USE_BOOST
/**
 * Method that handles loading a game from a file using Boost's serialization library
 * If the file does not exist throw an exception
(FileDoesntExistException)
 * @param File name where the game is loading
 */
...
/**
 * Method that deals with backing up the current game to a file using Boost serialization library
 * If the file to which the data was to be stored already throws an exception(FileAlreadyExistException)
 * @param filename to try to save the current game
...
/**
 * Method that handles loading a game from a file
 * If the file does not exist throw an exception (FileDoesntExistException)
 * @param filename File name where the game is loading
 */
void Domino::loadDomino(string filename) throw(FileDoesntExistException) {
	ifstream inputFileStream(filename.c_str());

	if (inputFileStream.is_open()) {
		// minimum score to finish the game
		...
		// number of players who were playing
		size_t numPlayers = 0;
...
/**
 * 
Method that handles backing up the current game to a file
 * 
If the file to save the data already exists throws an exception(FileAlreadyExistException)
 * @param filename filename to try to save the current game
 * @param 
overWrite flag indicating that if the file exists, it overwrites and launches except
 */
void Domino::saveDomino(string filename, bool overWrite) throw(FileAlreadyExistException) {
	if (!overWrite) {
		ifstream inputFileStream(filename.c_str());
